...
* 思路
   + 通过将cutlass gemm kernel 修改为按照指定列选取41列和另外一个tensor做矩阵乘法。提升了计算性能。
    41/4096                             4096          4096   
    x x √ x x ....           X    41           =             2048    
  并修改cutlasskernel实现。
...
**模型量化之后，压缩保存算法研究。**
* 背景
   + 在模型量化之后，我们需要保存模型weight，编程语言（如Python）不支持int4 数据类型，一般情况下int4数据是保存在FP16数据结构中，这样实际量化后模型大小和原始模型大小几乎没有变化，急需研究方法保存int4数据。
* 思路
  + 通过调研其他量化framwork的数据存储方式，选用uint32存储量化后的int4或者int8数据。即一个uint32存储8个int4,或者一个uint32存储4个int8的数据。设计了量化后模型压缩过程，并通过Python 基本操作（移位， 与或非等）完成量化后数据的存储。
** quad Tree 压缩算法实现 **
* 背景
